...
struct Parsley: Identifiable { // variables for map marker
    let id = UUID()
    let name: String
    let coordinates: CLLocationCoordinate2D
}

struct ParsleyMapView: View {
    
    @State private var region = MKCoordinateRegion(center: CLLocationCoordinate2D(latitude: -33.855856, longitude: 151.277165), span: MKCoordinateSpan(latitudeDelta: 0.05, longitudeDelta: 0.05)) // centre point map will show
    
    let ParsleyMarker = [
        Parsley(name: "Parsley Bay", coordinates: CLLocationCoordinate2D(latitude: -33.8522, longitude: 151.2781)) // marker location
    ]
    ...
    var body: some View {
        ...
        ZStack{
            ...
            Map(coordinateRegion: $region, interactionModes: .all, showsUserLocation: true, annotationItems: ParsleyMarker)
            {
                MapMarker(coordinate: $0.coordinates, tint: .blue) // displays map on screen
            ...
            VStack{
                ...
                ZStack{
                    ...
                    VStack{
                        ...
                        HStack(spacing: 10){
                            Text("Parsley Bay").font(.title3).fontWeight(.bold).foregroundColor(Color("homebutton")).offset(y: 25)
                            Text("|").font(.title).foregroundColor(Color("homebutton")).offset(y: 25)
                            Text("Vaucluse").font(.subheadline).fontWeight(.bold).foregroundColor(Color("homebutton")).offset(y: 25)
                        }
                        
                    }
                    
                    Image("Parsley").resizable().clipShape(Circle()).shadow(radius: 10).overlay(Circle().stroke(Color.blue, lineWidth: 2)).frame(width: 140, height: 140).offset(y: 150)
                }
...
struct ParsleyMapView_Previews: PreviewProvider {
