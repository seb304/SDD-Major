...
struct ParsleyView: View {
    var body: some View {
        ZStack {
            Image("background").resizable().aspectRatio(contentMode: .fill).ignoresSafeArea()
            
            VStack{
                Image("Parsley").resizable().padding().aspectRatio(contentMode: .fit)
                Spacer()
                HStack{
                    Text("Fish at Parsley Bay").font(.title3).fontWeight(.bold).foregroundColor(.black).padding().aspectRatio(contentMode: .fit)
                }
                HStack{
                ...
                Spacer(minLength: 20)
                NavigationLink(destination: ParsleyMapView()){
                    ZStack{
...
struct ParsleyView_Previews: PreviewProvider {
