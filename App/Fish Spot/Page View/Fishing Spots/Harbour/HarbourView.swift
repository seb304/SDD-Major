...
struct HarbourView: View {
    var body: some View {
        ZStack {
            ...
            VStack{
                Image("Harbour").resizable().padding().aspectRatio(contentMode: .fit)
                Spacer()
                HStack{
                    Text("Fish at Beulah St Wharf").font(.title3).fontWeight(.bold).foregroundColor(.black).padding().aspectRatio(contentMode: .fit)
                }
                HStack{
                    NavigationLink(destination: KingfishView()){
                        ZStack{
                            Rectangle().fill(Color.white).cornerRadius(20).opacity(0.5).frame(width: 175, height: 110)
                            Image("kingfish").resizable().aspectRatio(contentMode: .fit).frame(width: 150, height: 100)
                        }
                    }
                ...
                Spacer(minLength: 20)
                NavigationLink(destination: HarbourMapView()){
                    ZStack{
...
struct HarbourView_Previews: PreviewProvider {
